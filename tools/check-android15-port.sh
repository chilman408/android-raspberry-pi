...
require_match "aosptree/vendor/devices-community/gd_rpi4/boot/config.txt" \
  '^core_freq=250$' \
  "The Raspberry Pi mini-UART clock must remain stable across the kernel handoff"
reject_match "aosptree/vendor/devices-community/gd_rpi4/BoardConfig.mk" \
  'earlycon=uart8250,mmio32,0xfe215040|keep_bootcon' \
  "The early console must not reprogram the Raspberry Pi mini-UART during kernel handoff"
